...
bnb_config = BitsAndBytesConfig(
    load_in_8bit=True,
    llm_int8_enable_fp32_cpu_offload=True,  # ✅ Offload to CPU RAM
    llm_int8_threshold=6.0,
    llm_int8_skip_modules=["lm_head", "embed_tokens"]  # ✅ Prevents gradient errors
)
...
for name, param in model.named_parameters():
    if "lora" in name or "adapter" in name:  # 🔥 Check if it's LoRA params
...
training_args = TrainingArguments(
    gradient_checkpointing=False,  # ✅ Saves memory & allows larger batches
    ...
    label_names=["labels"],
    #use_cache=False  # ✅ Fixes checkpointing issue
)
...
backup_save_path = "sharing_path/t_LLama_model" # if you want to use the trained model on another device
